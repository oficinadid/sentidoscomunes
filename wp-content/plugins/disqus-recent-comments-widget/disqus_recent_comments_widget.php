...
class disqus_recent_comments_widget extends WP_Widget {
		...
		try {
		    ...
		    if( $response != false ) {
		    	...
		    	$response = @json_decode($response, true);
		      	...
		      	//check comment count
		      	if(count($comments) > 0) {
		      		if($comments != 'Has excedido el límite de consultas') {
			      		$this->echo_comments(
				                $comments, 
				                $api_key,
				                $style_params
	...
	protected function no_comments( $comment = false ) {
	    echo '<div id="disqus_rcw_comment_wrap"><span id="disqus_rcw_no_comments">No hay comentarios recientes</span>';
	...
	protected function echo_comments($comment, $api_key, $style_params) {
	    ...
	    //create html string
	    $recent_comments = '<div id="recent_comments" class="widget"><big>En discusión</big>';
		...
		if($comment != 'Invalid API key') {
		    ...
		    foreach($comment as $comment_obj) {	      
		      	...
		      	$comment_counter++;
		     	 //alternate class
		      	if($comment_counter % 2 !== 0) $wrap_class ="comment";
		      	else $wrap_class ="comment alt";
		      	...
		      	$thread_posts = $thread_info["posts"];
			    ...
			    $message = $this->shorten_comment(
		      	...
		      	$comment_html = '<div class="comment">
			      					<h4><strong>'.$author_name.'</strong> opina que:</h4>
			      					<a href="'.$thread_link.$comment_id.'" class="comments">'.$thread_posts.'</a>
			      					<p>'.$message.'</p>
			      					<a href="'.$thread_link.$comment_id.'" class="readmore">en '.$thread_title.'</a>
		                		</div>';
